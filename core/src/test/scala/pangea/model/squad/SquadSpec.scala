...
object SquadSpec extends ZIOSpecDefault {
  ...
  override def spec = suite("Squad")(
    ...
    test("найм: на первую свободную позицию, здоровым; повторно — нет; на полный отряд — нет") {
      val s1 = Squad.empty.hire(AllyKind.Human, lvl)
      val s2 = s1.hire(AllyKind.Murloc, lvl)
      val s3 = s2.hire(AllyKind.Gnome, lvl)
      val s4 = s3.hire(AllyKind.Human, lvl)
    ...
    test("герой на позиции 3: наём обходит его место") {
      val s = Squad(heroPos = 3).hire(AllyKind.Human, lvl).hire(AllyKind.Murloc, lvl).hire(AllyKind.Gnome, lvl)
    ...
    test("перестановка: на пустое — просто; на союзника — меняются; на героя — герой встаёт на прежнюю") {
      val s = Squad.empty.hire(AllyKind.Human, lvl).hire(AllyKind.Murloc, lvl) // герой 1, Йорген 2, Плюх 3
    ...
    test("герой шагает на позицию союзника — тот встаёт на его прежнюю") {
      val s = Squad.empty.hire(AllyKind.Human, lvl)
      val m = s.moveHero(2)
    ...
    test("свиток: из отряда вон, сутки в отлучке, потом возвращается — и встречается один раз") {
      val now = 1_000_000L
      val s   = Squad.empty.hire(AllyKind.Human, lvl).sentAway(AllyKind.Human, now)
    ...
    test("отдых восстанавливает всех; текущее не выше потолков") {
      val s = Squad.empty.hire(AllyKind.Human, lvl).update(AllyKind.Human)(_.copy(hp = 1L, armor = 0L, energy = 5L))
