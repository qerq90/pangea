...
object HeroExpSpec extends ZIOSpecDefault {
  ...
  override def spec = suite("Hero")(

    test("neededExpForLevel: последовательность 100, 200, 300, 500, 800, 1300, 2100, …") {
    ...
    test("HP нагрудника прибавляется к effectiveMaxHp") {
      ...
      val chest = Item(1L, "Нагрудник", 1L, Rarity.Green, ItemType.ChestPlate,
        attack = 0, accuracy = 0, concentration = 0, armor = 0, defence = 0, evasion = 0, hp = 50)
      val withChest = hero.copy(equipment = hero.equipment.copy(chestPlate = chest))
      assertTrue(withChest.effectiveMaxHp(0L) == hero.effectiveMaxHp(0L) + 50L)
    }
  )
}
