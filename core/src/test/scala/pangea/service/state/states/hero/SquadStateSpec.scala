...
object SquadStateSpec extends ZIOSpecDefault {
  ...
  override def spec = suite("SquadState")(
    ...
    test("enter → строй по позициям: герой, союзники, пустое место; кнопки — союзники и «Назад»") {
      for {
        ...
        scr <- renderer.sentScreens.map(_.last)
              ...
              assertTrue(scr.text.contains("2. Йорген Кремень — HP 1250/1250, броня 1500/1500")) &&
              assertTrue(scr.text.contains("3. Брамбл Медноус — HP 800/800, броня 1750/1750")) &&
    ...
    test("карточка: состояние и позиция, кнопки «На N (кто там)», «Выгнать», «К отряду»") {
      ...
      } yield assertTrue(scr.text.contains("Йорген Кремень (Человек, 🔥) — позиция 2.")) &&
              assertTrue(scr.text.contains("Атака 200, точность 1000, защита 400, уклонение 1000.")) &&
