...
object MaterialKind extends Enum[MaterialKind] {
  ...
  /** Цветок, который не цветёт: сам по себе бесполезен, зато в кубе Азата от
    * пыли самоцвета раскрывается стихией (см. [[RoseKind]]). Дороже прочих
  ...
  /** Травы данного ранга — все, включая те, что находят отдельным броском. */
  def herbsOfRank(rank: Int): IndexedSeq[MaterialKind] = values.filter(_.herbRank == rank)

  /** Травы, которые растут обычным порядком: из них берут находку на поляне и
    * горсти в походе за кладом. Нераскрытая роза сюда не входит — её ищут
    * отдельным броском и поштучно. */
  def commonHerbs(rank: Int): IndexedSeq[MaterialKind] =
    herbsOfRank(rank).filterNot(_ == UnopenedRose)
