 ...
 *  восьми надколотых камней (см. `CubeCraft`). Надевается в доп. слот — один из
 *  многих, что там носят, — и статов не даёт вовсе: вся его сила в ударе по
 *  всему полю боя, который он держит считанное число раз, а потом рассыпается в
 *  руках.
 *
