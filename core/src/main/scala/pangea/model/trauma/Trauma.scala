...
object Trauma {
  ...
  case object BruisedLeg extends Trauma {
    val name        = "Ушиб ноги"
    val severity    = Light
    val description = "Тяжело ушибленная нога сильно ограничивает мобильность. -5% к ловкости."
    val penalties   = TraumaPenalties(evasionPct = 0.05)
  }
  case object SmashedFinger extends Trauma {
    val name        = "Раздробленный палец"
    val severity    = Light
    val description = "Неудачный удар раздробил палец, держать что-то в руках стало болезненно. -5% к урону, -5% к силе."
    val penalties   = TraumaPenalties(atkPct = 0.05, strPct = 0.05)
  }
  case object AchingRib extends Trauma {
    val name        = "Ноющее ребро"
    val severity    = Light
    val description = "Серьёзный удар по телу оставил больное ребро, напоминающее о себе при каждом телодвижении. -5% к HP, -5% к телосложению."
    val penalties   = TraumaPenalties(hpPct = 0.05, vitPct = 0.05)
  }
  case object TornEar extends Trauma {
    val name        = "Порванное ухо"
    val severity    = Light
    val description = "Кровь залила голову и шею, что мешает ориентироваться в пространстве. -5% к точности, -5% к сосредоточению."
  ...
  case object PiercedArm extends Trauma {
    ...
    val severity    = Medium
    val description = "Обращение с оружием стало более трудным. -25% к урону, -25% к силе."
  ...
  case object DeepBellyGash extends Trauma {
    ...
    val severity    = Medium
    val description = "Глубокий порез повредил мышцу — болезненно даже стоять. -25% к HP, -25% к телосложению, -15% к броне, -15% к защите."
  ...
  case object BrokenNose extends Trauma {
    ...
    val severity    = Medium
    val description = "Тупым предметом был сломан нос — дышать стало затруднительно. -25% к точности, -25% к сосредоточению."
  ...
  case object KneecapDamage extends Trauma {
    ...
    val severity    = Heavy
    val description = "Хрупкая коленная чашечка повреждена. Боль с каждым движением. -50% к ловкости, -15% к броне, -15% к защите."
  ...
  case object DeepSideGash extends Trauma {
    ...
    val severity    = Heavy
    val description = "Глубокий разрез ободрал кожу, стали видны рёбра. -50% к HP, -50% к телосложению."
    val penalties   = TraumaPenalties(hpPct = 0.50, vitPct = 0.50)
  }
  case object SplitSkull extends Trauma {
    val name        = "Расколотый череп"
    val severity    = Heavy
    val description = "Множественные переломы черепа, сотрясение мозга, отёки. -50% к точности, -50% к сосредоточению, -75% к интеллекту."
