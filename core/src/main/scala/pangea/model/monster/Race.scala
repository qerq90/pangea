...
object Race extends Enum[Race] with DoobieEnum[Race] {
   ...
   *  травить. Проверяется при наложении эффектов в бою. */
  def immuneToDots(race: Race): Boolean = race == Elemental

  /** «Смертные» расы — всё, кроме элементалей. Именно ИЗ НИХ выбирают расу при
   *  создании персонажа, генерации обычных мобов, могил и заданий трактирщика:
   *  элементаль не игровая раса и появляется только своим событием, поэтому
   *  везде, где раньше брали `values`, брать нужно этот список. */
  val mortals: IndexedSeq[Race] = values.filterNot(_ == Elemental)
