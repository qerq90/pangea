...
object Element extends Enum[Element] {
  ...
  // ── Молния ────────────────────────────────────────────────────────────────────
  /** Сколько % ПОТОЛКА энергии цели сжигает прок Молнии. Раньше выжигание было
   *  флейвором — у мобов не было рабочего пула энергии; теперь он есть, и прок
   *  реально отодвигает следующее умение. */
  val LightningEnergyBurnPct: Long = 30L
