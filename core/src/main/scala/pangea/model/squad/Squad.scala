...
/** Союзник в отряде: кто, на какой позиции и что с ним сейчас. Потолки считаются
 *  от уровня героя ([[AllyKind.stats]]), здесь — только текущее. */
final case class Ally(kind: AllyKind, position: Int, hp: Long, armor: Long, energy: Long) {
...
object Ally {
  ...
  implicit val decoder: Decoder[Ally] = (c: HCursor) =>
    ...
    } yield Ally(kind, position, hp, armor, energy)
}
 ...
 *  когда вернётся в таверну (ключ — вид). Хранится в `heroes.squad_data`. */
final case class Squad(
  ...
  def isAway(kind: AllyKind, nowMs: Long): Boolean = away.get(kind.entryName).exists(_ > nowMs)

  /** Кто вернулся из отлучки к этому моменту — им нужна реплика о свитке. */
  def returned(nowMs: Long): List[AllyKind] =
    away.collect { case (k, until) if until <= nowMs => AllyKind.withNameOption(k) }.flatten.toList

  /** Стереть запись об отлучке (реплика показана). */
  def welcomeBack(kind: AllyKind): Squad = copy(away = away - kind.entryName)

  /** Первая свободная позиция, если есть. */
  def freePosition: Option[Int] =
    (1 to AllyRates.Positions).find(p => p != heroPos && allyAt(p).isEmpty)

  /** Нанять: на первую свободную позицию, полностью здоровым. */
  def hire(kind: AllyKind, lvl: Long): Squad =
    if (has(kind)) this
    else freePosition.fold(this)(p => copy(allies = allies :+ Ally(kind, p, 0L, 0L, 0L).restored(lvl)))
...
object Squad {
  ...
  implicit val decoder: Decoder[Squad] = (c: HCursor) =>
    ...
    } yield Squad(heroPos, allies, away)
