  ...
  private def resolveElementProcs(hero: Hero, battle: SoloPveBattle): Task[(SoloPveBattle, Vector[String])] = {
    ...
    if (elems.isEmpty) ZIO.succeed((battle, Vector.empty))
      ...
      ZIO.foreach(Element.values.filter(elems.contains).toList) { e =>
        ...
        // умение — моб молчит два своих хода подряд. Одиночные проки обеих стихий
        // при этом ОСТАЮТСЯ: комбо идёт им в довесок, а не вместо них.
        if (lightCold) {
...
object BattleState {
  ...
  /** На сколько тиков комбо запирает умение моба. Гаснут ДВА каста: тот, что был
    * бы ответом на комбо, и следующий за ним. Тройка, а не двойка, потому что тик
    * буфов идёт в начале хода моба и сразу съедает один заряд. */
  val ComboSkillBlockTurns: Int = 3
