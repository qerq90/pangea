...
/**
 * Трактирщик. Принимает квестовые предметы: забирает ПЕРВЫЙ подходящий трофей из
 * инвентаря (тип `Trophy`, нужная раса) и начисляет опыт по формуле
 * `5 + Ур.трофея × коэффициент(редкость)`, округляя вверх.
 */
case class InnkeeperState(
  ...
  private def turnInQuest(user: User, renderer: Renderer): Task[StateType] =
    for {
      ...
      _ <- data.flatMap(_.active) match {
             case None =>
               renderer.show(user, Screen(content.text("innkeeper.noActive"), Nil)) *> showMenu(user, renderer)
             case Some(raceName) =>
               for {
                 inv <- inventoryRepo.get(hero.id).mapError(e => new Throwable(e.toString))
                 matching = inv.items.data.find(i => i.itemType == ItemType.Trophy && i.race.contains(raceName))
                 _ <- matching match {
                        case None =>
                          renderer.show(user, Screen(
                            content.format("innkeeper.noItem", "race" -> Race.withName(raceName).toString), Nil)) *>
                        ...
                        case Some(item) =>
                          ...
                          inventoryRepo.removeItem(item.id, hero.id).mapError(e => new Throwable(e.toString)) *>
                            heroDao.updateExpAndLevel(user.userId, leveled.exp, leveled.lvl, leveled.upgradePoints) *>
                            heroDao.writeQuestData(user.userId, data.get.copy(active = None).asJson) *>
                            renderer.show(user, Screen(content.format("innkeeper.completed",
                              "item" -> item.name, "exp" -> expGained.toString), Nil)) *>
                            ZIO.when(leveled.lvl > hero.lvl)(renderer.show(user, Screen(
                              content.format("innkeeper.levelUp", "level" -> leveled.lvl.toString), Nil))) *>
                            showMenu(user, renderer)
                      }
               } yield ()
           }
  ...
  // Опыт за трофей: 5 + Ур.трофея × коэффициент редкости (factorR), округление вверх.
  private def questExp(trophy: Item): Long =
    math.ceil(5 + trophy.lvl * trophy.rarity.factorR).toLong
