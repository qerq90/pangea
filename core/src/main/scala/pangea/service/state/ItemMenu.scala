...
object ItemMenu {
   ...
   *  ([[NavRow]]) — туда уходят «Назад» + «Пред.» + «След.». На средних
   *  страницах в нав-ряду все три кнопки, и при 9 предметах ВК-клиент стабильно
   *  глотает такое сообщение (10 рядов + 3-кнопочный последний ряд) — выявлено
   *  по логам, в `send_failures` ошибок нет. Поэтому держим 8 предметов на
   *  странице: 8 + nav = 9 рядов, безопасный запас. */
  val DefaultPageSize: Int = 8

  /** Индекс ряда с навигационными кнопками: сразу после последнего предмета. */
  val NavRow: Int = DefaultPageSize
