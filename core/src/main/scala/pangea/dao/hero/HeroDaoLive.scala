...
class HeroDaoLive(xa: Transactor[Task]) extends HeroDao {
  ...
  override def updateSilver(userId: UserId, silver: Long): Task[Unit] =
    Queries.updateSilver(userId, silver).update.run.transact(xa).unit
  ...
  override def updateSquad(userId: UserId, squad: pangea.model.squad.Squad): Task[Unit] =
    Queries.updateSquad(userId, squad.asJson).update.run.transact(xa).unit

  override def updateRunes(userId: UserId, runes: pangea.model.rune.RuneData): Task[Unit] =
    Queries.updateRunes(userId, runes.asJson).update.run.transact(xa).unit
  ...
  override def writeCardSellerData(userId: UserId, data: Json): Task[Unit] =
    Queries.writeCardSellerData(userId, data).update.run.transact(xa).unit

  override def readCardSellerData(userId: UserId): Task[Option[Json]] =
    Queries.readCardSellerData(userId).query[Option[Json]].unique.transact(xa)
  ...
  override def writeAzatData(userId: UserId, data: Json): Task[Unit] =
    Queries.writeAzatData(userId, data).update.run.transact(xa).unit

  override def readAzatData(userId: UserId): Task[Option[Json]] =
    Queries.readAzatData(userId).query[Option[Json]].unique.transact(xa)

  override def writeLoreData(userId: UserId, data: Json): Task[Unit] =
    Queries.writeLoreData(userId, data).update.run.transact(xa).unit
  ...
  override def readNpcQuests(userId: UserId): Task[Option[Json]] =
    Queries.readNpcQuests(userId).query[Option[Json]].unique.transact(xa)
