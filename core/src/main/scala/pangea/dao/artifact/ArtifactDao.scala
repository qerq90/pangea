...
import pangea.model.artifact.{Artifact, HeroArtifacts}
...
class ArtifactDaoLive(xa: Transactor[Task]) extends ArtifactDao {
  ...
  override def getOrCreate(heroId: HeroId): Task[HeroArtifacts] =
    (for {
      _ <- sql"""insert into hero_artifacts(hero_id, casket_items, bag_items)
                 values($heroId, ${Items(List.empty[Item])}, ${Items(List.empty[Item])})
      ...
      row <- sql"""select hero_id, casket_tier, casket_charges, casket_items,
               ...
               .query[(HeroId, Int, Int, Items, Int, Int, Items)].unique
    } yield {
      val (id, cTier, cCharges, cItems, bTier, bCharges, bItems) = row
      HeroArtifacts(id, Artifact(cTier, cCharges, cItems), Artifact(bTier, bCharges, bItems))
    }).transact(xa)
